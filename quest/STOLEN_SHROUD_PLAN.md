...
### Beat 2: Tracking the thief (Urban Investigation)
...
* **Branching Approaches:**
  ...
  * **Intimidation / CHA:** Squeeze an off-duty runner by the dice cellar door, who quickly gives up the thief's hideout: an abandoned smoking cellar behind the drinking house.
* **Convergence:** You corner the thief in the dark cellar beneath Lamp Stair. He's already splitting the coins onto a barrel-head with an accomplice.

### Beat 3: Recovering the Purse (The Confrontation)

* **Bottleneck:** The thief draws an iron-tipped cudgel, backed against the wall.
* **Branching Approaches:**
...
* **Convergence:** The thieves yield or drop. You retrieve the intact shroud-purse (`40 copper bits` / `4 silver marks`) and find an extra piece of stolen jewelry on the thief's neck.

I don't see a failure path here, so I'm assuming failure means, that a fight starts, and the play is facing what would be the equaivalent of 2 lookouts in the combat library? A weaker fight sicne they are found.
...
* **Consequences:**
  * The widow presses a `Phial of Saint Althea's Water` into your hands — cistern-water the parish keeps for those who go out on the river, steeped with wild angelica and mountain arnica (`2d4+2 HP`, stackable).(Im thinking this is also a good place to purchase thiese health potions at 1 silver piece each(for completing the quest and returning all the funds), probably limit these 1 purcase of it a month and important returning location for the play. We'd have the named this friar at that point since they'd be an important NPC.)
...
* Pocket the entire purse and sell the thief's stolen neck-chain at the Lamp Stair pawnbroker.
* **Consequences:** Highest immediate cash payout (`+6 silver marks` total), but the shrine queue falls quiet when you pass. (This is also a good place to purchase these potions, but at a higher price of 1 silver mark each, probably limit these 1 purcase of it a month)
...
2. **The reward is the shrine's own medicine, and the old waterman's charm is cut.** The phial is an existing item — `has_althea_phial`, "Phial of Saint Althea's Water", restores `2d4+2 HP` — and it is already given away as a quest reward by Deacon Corbel in Alderford after the flour delivery (`alderford.txt:3205-3211`). No vendor stocks it and it has no sell rates, so it can be neither bought nor pawned; its price is carried as catalog `retail`, which was raised from 10 to 50 copper in `tools/gear_catalog.json` (the generator wrote it through to `equipment.txt:756`). At **5 silver** the widow's gift is worth more than the 40-copper purse it comes out of, so Ending A is no longer the cheapest path by raw value. It stacks, and the engine already has the helper: `*gosub_scene equipment grant_gear althea_phial 1` sets `has_althea_phial` for a first phial and puts any extra copy into `spare_althea_phial` (`equipment.txt:916-930`, driven by the `gear_stack true` the phial declares at `:759`), the dose label shows the count as "×N" (`inventory-data.js:150`, `choicescript_stats.txt:264`; the counter is created at `startup.txt:1286`), and `use_consumable` spends a spare when the dose is drunk (`equipment.txt:1049-1052`). Two prose constraints to respect: at full HP the phial is refused and not consumed, and out of combat it shares the once-a-day instant-heal limit (`last_mundane_treatment_day`) with the Dredge-End hedge-doctor. The draft's "Waterman's Carved Bone Charm (`+1 WIS` against cold/river exposure)" is dropped because the neck slot already belongs to Elspeth's Braided Weir-Knot, which is *the* canonical waterman's charm (`elspeth_weir_knot`, `equipment.txt:481`), and `codex_weir_knots` already unlocks with it in Alderford (`alderford.txt:3242`) — a second charm would have fought for the slot and could not have unlocked that entry.
